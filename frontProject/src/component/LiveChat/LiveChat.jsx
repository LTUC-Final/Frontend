...
useEffect(() => {
  ...
  // 👇 هذا الجزء يراقب المستخدم إذا طلع أو نزل
  useEffect(() => {
    const container = messageContainerRef.current;
    if (!container) return;
  ...
  return (
    <div className="max-w-md mx-auto p-4 rounded-2xl bg-[#FFF6E9] border border-[#F5C45E]/60 shadow-[0_18px_40px_rgba(16,46,80,0.15)]">
      ...
      <div
        ...
        className="border border-[#F5C45E]/70 rounded-2xl p-3 h-64 overflow-y-auto mb-4 flex flex-col gap-2 bg-white/70 backdrop-blur-sm shadow-inner"
      >
        {messages.map((msg, idx) => {
          const isSender =
          ...
          return (
            <div
              key={idx}
              className={`px-3 py-2 rounded-2xl text-sm leading-relaxed max-w-[80%] shadow-[0_6px_14px_rgba(16,46,80,0.10)] ${
                isSender
                  ? "bg-gradient-to-r from-[#BE3D2A] to-[#E78B48] text-[#FFF6E9] self-end rounded-tr-sm"
                  : "bg-white text-[#102E50] border border-[#F5C45E]/60 self-start rounded-tl-sm"
              }`}
